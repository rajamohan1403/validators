...
def check_for_a(value):
    if value[0].upper=='A':
        raise forms.ValidateError('should not start with A')

def check_for_len(v):
    if len(v)>=5:
        raise forms.ValidateError('length should not be more than 5')

class NameForm(forms.Form):
    name=forms.CharField(max_length=100,validators=[check_for_a,validators.MaxLengthValidator(5)])
    fathername=forms.CharField(max_length=100,validators=[check_for_a])
